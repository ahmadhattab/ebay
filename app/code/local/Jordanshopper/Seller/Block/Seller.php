    	...
    	{        
	        ...
	        foreach ($subCategories as $subCategoryId) {
	           ...
	           if ($category->getIsActive()) {
	                $output .= "<li class='x-tree-node'><input type='checkbox' value='{$category->getId()}'>{$category->getName()}";                               
	           }
	            if($category->hasChildren()){                
	                $output .= "<ul>";
	                $subCategoriesList = $category->getChildrenCategories();
	                foreach ($subCategoriesList as $subcategory){
	                    $output .= "<li class='menu-item-2'><input type='checkbox' value='{$subcategory->getId()}'>{$subcategory->getName()}</li>";                               
	                }
                            $output .= "</ul></li>";
                    }else{
                        $output .= "</li>";
